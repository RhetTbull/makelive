...
PYPI_VERSION=$(python $DIR/scripts/get_latest_pypi_version.py makelive)
...
bash scripts/pyapp-runner.sh m1 makelive $VERSION

echo "Building version $VERSION for Intel"
bash scripts/pyapp-runner.sh macbook makelive $VERSION
